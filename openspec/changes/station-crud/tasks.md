...
- [x] 5.1 Update `CreateStationReq` in `Server/Json.hs`: drop `cstrId`, change `cstrName` from `String` to `Text`, add `cstrMinStaff` and `cstrMaxStaff` as `Int` with parser-level defaults of 1 (`.:? "minStaff" .!= 1`)
- [x] 5.2 Add `RenameStationReq` JSON type in `Server/Json.hs`
- [x] 5.3 Add `Station` JSON instance (ToJSON) for the list endpoint response (name, minStaff, maxStaff)
- [x] 5.4 Add `StationReferencesResp` JSON type mirroring `SkillReferencesResp` pattern
- [x] 5.5 Update station route types in `Server/Api.hs`: change `Capture "id" Int` to `Capture "name" Text` on all station routes; add rename (`PUT` with `RenameStationReq`) and force-delete (`DELETE .../force`) routes
- [x] 5.6 Add `resolveStationName :: Repository -> Text -> Handler StationId` in `Handlers.hs`
- [x] 5.7 Update `lookupStationName` to return `Text` and destructure `Station` records
- [x] 5.8 Update `handleListStations` to return `[Station]` (name, minStaff, maxStaff — no IDs)
- [x] 5.9 Update `handleCreateStation` to use new `CreateStationReq` (name, optional minStaff/maxStaff)
- [x] 5.10 Update `handleDeleteStation` to resolve by name and use `safeDeleteStation`; return 409 with `StationReferencesResp` if blocked
- [x] 5.11 Add `handleForceDeleteStation` that resolves name then calls `executeCommandText` with `"station force-delete <name>"`
- [x] 5.12 Add `handleRenameStation` that resolves name, calls `SW.renameStation`, logs via `logRest`
- [x] 5.13 Update `handleSetStationHours` and `handleSetStationClosure` to resolve by name
- [x] 5.14 Wire all updated/new handlers into the server in `Server/Api.hs`

## 6. React Admin UI

- [x] 6.1 Create `web/src/api/stations.ts` with `fetchStations`, `createStation`, `deleteStation`, `forceDeleteStation`, `renameStation`
- [x] 6.2 Create `web/src/components/StationsListPage.tsx` with station table (Name linked to detail, Min Staff, Max Staff, Delete column), create form (name only), safe/force-delete modal following Skill pattern
- [x] 6.3 Create `web/src/components/StationDetailPage.tsx` — minimal: name (editable inline with save/rename), min/max staff (read-only)
- [x] 6.4 Add `/stations` and `/stations/:name` routes to `AppShell.tsx`
- [x] 6.5 Add "Stations" link to `Sidebar.tsx`
- [x] 6.6 Use `useEntityEvents("station", loadData)` on both pages for auto-reload

## 7. CLI Display

- [x] 7.1 Add `displayStationView` to `CLI/Display.hs` showing: name (with ID), min/max staff, hours/closures/multi-station hours (reusing `showStationHoursNote` formatting), required skills, worker preferences; "Hours: all" if no hours configured

## 8. Verify

- [x] 8.1 Build and fix all compiler warnings
- [x] 8.2 Run tests and fix failures
- [x] 8.3 Run demo replay and verify it completes successfully
- [x] 8.4 Test station CRUD through the web UI in browser
