...
When a mutating command is executed within a draft session while hints are active, the system SHALL clear the hint session and notify the user. This prevents stale hint sessions that no longer reflect the current state.

#### Scenario: Mutating command clears hint session
- **WHEN** hint session has 2 hints and user executes `worker grant-skill 2 1`
- **THEN** system clears the hint session and displays "Hint session cleared due to data change. Use what-if commands to start a new exploration."
