# User Create No Worker

Adds an optional `--no-worker` flag on `user create` to create users with `worker_status = 'none'`. Without the flag, users are created as active workers by default.
