...
Provides `worker delete <name>` to permanently remove the worker concept (sets `worker_status = 'none'`) when no references remain, and `worker force-delete <name>` to cascade-clear all worker rows. The user account itself is preserved in both cases.
