...
The system SHALL delete every persisted hint session associated with a draft when that draft is committed or discarded — not only the session belonging to the caller. Cleanup SHALL happen in `Service.Draft`, so that a commit or discard over REST cleans up exactly as a CLI one does.
