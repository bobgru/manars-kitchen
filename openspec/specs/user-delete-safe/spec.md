...
Makes `user delete <id>` block when the user is still a worker, and adds `user force-delete <id>` to cascade-remove a user along with all worker references. Operators must deactivate or delete the worker first before the safe delete will succeed.
